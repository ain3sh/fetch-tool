...
**deep-fetch** is a Model Context Protocol (MCP) server that provides web content fetching capabilities for AI assistants. It converts HTML pages to clean markdown using Mozilla Readability and optionally processes images using Sharp.
...
### Source Structure
- **Core logic**: All functionality is in `src/index.ts`
- **Type definitions**: External module types in `src/types.d.ts`
- **Build output**: Compiled to `dist/index.js` for distribution
...
**Server Configuration (src/index.ts:72+):**
```typescript
...
### Parameter Handling
Simplified schema exposes only essential params to LLM (src/index.ts:584+):
